...
/**
 * Backfill tracks.isrc from a fresh /me/tracks payload, for rows that already
 * exist but have isrc = NULL.
 *
 * Why this is needed: upsertTracks uses ON CONFLICT DO NOTHING (CLAUDE.md §4),
 * so rows inserted before migration 0004 added the isrc column keep isrc = NULL
 * forever — re-syncing never updates them. But Spotify's /me/tracks payload DOES
 * carry external_ids.isrc, so every sync can fill the gap. The backfill_track_isrcs
 * RPC (migration 0016) updates ONLY null isrc rows, so a present value is never
 * overwritten and no other column or enrichment marker is touched.
 *
 * (The old server-side fallback — MusicBrainz recording search by Spotify URL,
 * lib/inngest/functions/library-backfill-isrc.js — was retired: it matched spam
 * recordings titled with a Spotify URL, returning junk MBIDs and zero ISRCs.)
 *
 * @param {object[]} spotifyTracks  raw /me/tracks track objects
 * @param {object} [client]
 * @returns {Promise<number>} number of rows backfilled
 */
export async function backfillTrackIsrcs(spotifyTracks, client = createAdminClient()) {
    const pairs = spotifyTracks
        .filter((t) => t?.id && t.external_ids?.isrc)
        .map((t) => ({ spotify_id: t.id, isrc: t.external_ids.isrc }));
    if (pairs.length === 0) return 0;

    let filled = 0;
    for (const batch of chunk(pairs, QUERY_CHUNK)) {
        const { data, error } = await client.rpc("backfill_track_isrcs", {
            p_pairs: batch,
        });
        if (error) throw new Error(`backfillTrackIsrcs failed: ${error.message}`);
        filled += Number(data) || 0;
    }
    return filled;
}
...
/**
 * Upsert artist rows. We only persist id + name: Spotify's /artists endpoint
 * (the source of genres/popularity) is 403-blocked for this app, so those
 * columns were dropped (migration 0012). Callers pass whatever artist objects
 * they have — extra fields are ignored.
 *
 * @param {object[]} spotifyArtists  raw artist objects (id + name used)
 * @param {object} [client]
 */
...
// ---------------------------------------------------------------------------
// Layer 1.5 — artist genre floor (migration 0014).
//
// Mirrors the Last.fm TRACK helpers above, but keyed by artist and writing to
// artist_tags. Enrich each artist ONCE (artists.tags_enriched_at marks the
// pass); every track by that artist then inherits these genres downstream when
// its own track-level tags are sparse. See lib/inngest/functions/artist-enrich.js
// and lib/lastfm.js:fetchArtistTopTags.
// ---------------------------------------------------------------------------

// Cap the floor to the strongest few genres per artist. Past the top handful,
// Last.fm artist tags drift into noise ("seen live", decades, one-off moods)
// that would dilute a track's inherited signal.
const MAX_ARTIST_TAGS = 8;

/**
 * Given a batch of artist spotify_ids, return the subset whose artists row
 * exists AND has tags_enriched_at IS NULL — i.e. the artists the floor worker
 * should still process. Mirrors getUnenrichedTrackIds. Backed by
 * artists_tags_unenriched_idx (migration 0014).
 *
 * @param {string[]} artistSpotifyIds
 * @param {object} [client]
 * @returns {Promise<string[]>}
 */
export async function getUnenrichedArtistIds(artistSpotifyIds, client = createAdminClient()) {
    if (artistSpotifyIds.length === 0) return [];

    const out = [];
    for (const batch of chunk(artistSpotifyIds, QUERY_CHUNK)) {
        const { data, error } = await client
            .from("artists")
            .select("spotify_id")
            .in("spotify_id", batch)
            .is("tags_enriched_at", null);
        if (error) throw new Error(`getUnenrichedArtistIds failed: ${error.message}`);
        for (const row of data) out.push(row.spotify_id);
    }
    return out;
}
...
/**
 * Bounded-scan queue for the artist-floor cron: artists still needing a Last.fm
 * pass. Unlike the track ISRC backfill, this needs no per-user Spotify token —
 * artist.getTopTags keys on the artist NAME, which we already store — so the
 * cron can run immediately. Returns name alongside spotify_id so the worker can
 * call Last.fm without a second lookup. Backed by artists_tags_unenriched_idx.
 *
 * @param {{limit?: number}} [opts]
 * @param {object} [client]
 * @returns {Promise<Array<{spotifyId: string, name: string}>>}
 */
export async function getArtistsNeedingTags({ limit = 200 } = {}, client = createAdminClient()) {
    const { data, error } = await client
        .from("artists")
        .select("spotify_id, name")
        .is("tags_enriched_at", null)
        .limit(limit);
    if (error) throw new Error(`getArtistsNeedingTags failed: ${error.message}`);
    return (data ?? []).map((r) => ({ spotifyId: r.spotify_id, name: r.name }));
}

/**
 * Write the Last.fm artist-tag floor for ONE artist. Pattern mirrors
 * upsertLastfmTagsForTrack: normalize + dedup, atomic tags upsert, resolve PKs,
 * atomic upsert into artist_tags (source='lastfm'). Capped to MAX_ARTIST_TAGS.
 *
 * @param {string} artistSpotifyId
 * @param {{name: string, count: number}[]} lastfmTags  output of fetchArtistTopTags
 * @param {object} [client]
 */
export async function upsertArtistTags(artistSpotifyId, lastfmTags, client = createAdminClient()) {
    // (1) Normalize + dedup. byName: normalized-name → max raw count seen.
    const byName = new Map();
    for (const t of lastfmTags) {
        const norm = normalizeTag(t?.name);
    ...
    // Keep only the strongest MAX_ARTIST_TAGS by count (ties broken by name for
    // determinism). Trimming here keeps the floor focused on real genres.
    const topTags = [...byName.entries()]
        .sort((a, b) => (b[1] - a[1]) || a[0].localeCompare(b[0]))
        .slice(0, MAX_ARTIST_TAGS);
    const names = topTags.map(([name]) => name);

    // (2) Ensure tag rows exist (atomic upsert, CLAUDE.md §4).
    for (const batch of chunk(names.map((name) => ({ name })), QUERY_CHUNK)) {
        const { error } = await client
            .from("tags")
            .upsert(batch, { onConflict: "name", ignoreDuplicates: true });
        if (error) throw new Error(`upsertArtistTags tag insert failed: ${error.message}`);
    }
    ...
    for (const batch of chunk(names, QUERY_CHUNK)) {
        const { data, error } = await client
            ...
            .select("id, name")
            .in("name", batch);
        if (error) throw new Error(`upsertArtistTags tag lookup failed: ${error.message}`);
        for (const row of data) tagIdMap.set(row.name, row.id);
    }

    // (4) Resolve artistSpotifyId → artist PK.
    const { data: artistRow, error: aErr } = await client
        .from("artists")
        .select("id")
        .eq("spotify_id", artistSpotifyId)
        .maybeSingle();
    if (aErr) throw new Error(`upsertArtistTags artist lookup failed: ${aErr.message}`);
    if (!artistRow) return;

    // (5) Build junction rows. Clamp count to smallint 0–100 defensively.
    const rows = topTags
        .map(([name, lastfm_count]) => ({
    ...
    const { error } = await client
        .from("artist_tags")
        .upsert(rows, { onConflict: "artist_id,tag_id,source", ignoreDuplicates: true });
    if (error) throw new Error(`upsertArtistTags junction insert failed: ${error.message}`);
}

/**
 * Stamp tags_enriched_at = NOW() for artists the floor worker processed — hits
 * AND empty-result misses alike. Marking misses keeps artists_tags_unenriched_idx
 * draining instead of re-querying tag-less artists forever. Mirrors
 * markTracksEnrichedSuccess.
 *
 * @param {string[]} artistSpotifyIds
 * @param {object} [client]
 */
export async function markArtistsTagsEnriched(artistSpotifyIds, client = createAdminClient()) {
    if (artistSpotifyIds.length === 0) return;
    ...
    for (const batch of chunk(artistSpotifyIds, QUERY_CHUNK)) {
        const { error } = await client
            .from("artists")
            .update({ tags_enriched_at: enrichedAt })
            .in("spotify_id", batch);
        if (error) throw new Error(`markArtistsTagsEnriched failed: ${error.message}`);
    }
}


// ---------------------------------------------------------------------------
// Layer 1 — external track-level genre sources (Deezer, iTunes; migration 0015).
//
// Coarse per-track genres pulled from streaming catalogs, written to
// track_tags_external with a `source`. These rescue the no-ISRC tail that
// Last.fm track tags and the ISRC-keyed Wikidata layer miss. Drained by the
// backfill crons in lib/inngest/functions/; see lib/deezer.js, lib/itunes.js.
// ---------------------------------------------------------------------------

// Shared queue lookup: tracks that still need a given external source, each with
// its primary artist name (lowest track_artists.position) so the worker can do a
// fuzzy lookup without a second query. `pendingColumn` is the per-source marker.
async function getTracksNeedingExternal(pendingColumn, { limit, withIsrc }, client) {
    const cols = withIsrc
        ? "spotify_id, isrc, name, track_artists(position, artists(name))"
        : "spotify_id, name, track_artists(position, artists(name))";
    const { data, error } = await client
        .from("tracks")
        .select(cols)
        .is(pendingColumn, null)
        .limit(limit);
    if (error) {
        throw new Error(`getTracksNeedingExternal(${pendingColumn}) failed: ${error.message}`);
    }
    return (data ?? []).map((t) => {
        const primary = (t.track_artists ?? [])
            .slice()
            .sort((a, b) => a.position - b.position)[0];
        return {
            spotifyId: t.spotify_id,
            isrc: withIsrc ? (t.isrc ?? null) : null,
            name: t.name,
            artist: primary?.artists?.name ?? null,
        };
    });
}

/** Deezer queue (carries ISRC for the exact path). Backed by tracks_deezer_pending_idx. */
export async function getTracksNeedingDeezer({ limit = 50 } = {}, client = createAdminClient()) {
    return getTracksNeedingExternal("deezer_checked_at", { limit, withIsrc: true }, client);
}

/** iTunes queue (fuzzy only — no ISRC needed). Backed by tracks_itunes_pending_idx. */
export async function getTracksNeedingItunes({ limit = 40 } = {}, client = createAdminClient()) {
    return getTracksNeedingExternal("itunes_checked_at", { limit, withIsrc: false }, client);
}

/**
 * Write external genre tags for ONE track under a given source. Mirrors
 * upsertLastfmTagsForTrack: normalize + dedup via normalizeTag, atomic tags
 * upsert, resolve PKs, atomic upsert into track_tags_external. No count column —
 * these sources return a flat genre list.
 *
 * @param {string} trackSpotifyId
 * @param {"deezer"|"itunes"} source
 * @param {string[]} genreNames
 * @param {object} [client]
 */
export async function upsertExternalTagsForTrack(
    trackSpotifyId,
    source,
    genreNames,
    client = createAdminClient(),
    ...
    const names = [
        ...new Set((genreNames ?? []).map((n) => normalizeTag(n)).filter((n) => n.length > 0)),
    ];
    ...
    for (const batch of chunk(names.map((name) => ({ name })), QUERY_CHUNK)) {
        const { error } = await client
            .from("tags")
            .upsert(batch, { onConflict: "name", ignoreDuplicates: true });
        if (error) throw new Error(`upsertExternalTagsForTrack tag insert failed: ${error.message}`);
    }
    ...
    for (const batch of chunk(names, QUERY_CHUNK)) {
        const { data, error } = await client
            ...
            .select("id, name")
            .in("name", batch);
        if (error) throw new Error(`upsertExternalTagsForTrack tag lookup failed: ${error.message}`);
        for (const row of data) tagIdMap.set(row.name, row.id);
    }
    ...
    const { data: trackRow, error: tErr } = await client
        ...
        .eq("spotify_id", trackSpotifyId)
        .maybeSingle();
    if (tErr) throw new Error(`upsertExternalTagsForTrack track lookup failed: ${tErr.message}`);
    if (!trackRow) return;

    const rows = names
        .map((name) => ({ track_id: trackRow.id, tag_id: tagIdMap.get(name), source }))
        .filter((r) => r.tag_id !== undefined);
    if (rows.length === 0) return;

    const { error } = await client
        .from("track_tags_external")
        .upsert(rows, { onConflict: "track_id,tag_id,source", ignoreDuplicates: true });
    if (error) throw new Error(`upsertExternalTagsForTrack junction insert failed: ${error.message}`);
}

// Stamp a per-source checked marker for tracks the worker consulted — hits AND
// misses — so the pending queue drains and we never re-query a dead lookup.
async function markExternalChecked(column, spotifyIds, client) {
    if (spotifyIds.length === 0) return;
    const checkedAt = new Date().toISOString();
    for (const batch of chunk(spotifyIds, QUERY_CHUNK)) {
        const { error } = await client
            .from("tracks")
            .update({ [column]: checkedAt })
            .in("spotify_id", batch);
        if (error) throw new Error(`markExternalChecked(${column}) failed: ${error.message}`);
    }
}

export async function markTracksDeezerChecked(spotifyIds, client = createAdminClient()) {
    return markExternalChecked("deezer_checked_at", spotifyIds, client);
}
export async function markTracksItunesChecked(spotifyIds, client = createAdminClient()) {
    return markExternalChecked("itunes_checked_at", spotifyIds, client);
}
 ...
 *
 * Returns each row's isrc AND musicbrainz_recording_id so the worker can try
 * the by-ISRC lookup first and fall back to by-MBID (wdt:P4404) for ISRC-less
 * tracks. Rows with neither identifier are still returned (both null) so the
 * worker can mark them enriched-as-miss instead of leaving them queued.
 *
 * @param {string[]} spotifyIds
 * @param {object} [client]
 * @returns {Promise<Array<{spotifyId: string, isrc: string | null, mbid: string | null}>>}
 */
/**
 * Bounded-scan queue for the Wikidata backfill cron: tracks past Layer 1
 * (enriched_at set) that still need a Wikidata pass (wikidata_enriched_at NULL).
 * Returns spotify_ids only — the worker resolves isrc/mbid itself. Backed by
 * tracks_wikidata_pending_idx.
 *
 * @param {{limit?: number}} [opts]
 * @param {object} [client]
 * @returns {Promise<string[]>}
 */
export async function getTracksNeedingWikidataIds({ limit = 100 } = {}, client = createAdminClient()) {
    const { data, error } = await client
        .from("tracks")
        .select("spotify_id")
        ...
        .limit(limit);
    if (error) throw new Error(`getTracksNeedingWikidataIds failed: ${error.message}`);
    return (data ?? []).map((r) => r.spotify_id);
}
...
export async function getTracksNeedingWikidata(spotifyIds, client = createAdminClient()) {
    ...
    for (const batch of chunk(spotifyIds, QUERY_CHUNK)) {
        const { data, error } = await client
            .from("tracks")
            .select("spotify_id, isrc, musicbrainz_recording_id")
            .in("spotify_id", batch)
            .not("enriched_at", "is", null)
            .is("wikidata_enriched_at", null);
        ...
        for (const row of data) {
            out.push({
                ...
                isrc: row.isrc ?? null,
                mbid: row.musicbrainz_recording_id ?? null,
            });
        }
    }
    return out;
...
/**
 * Stamp musicbrainz_checked_at = NOW() for spotify_ids the backfill consulted
 * MusicBrainz about. Used for the "not_found" path (the "ok" path stamps it via
 * updateTrackIsrcAndMbid). Bad-data skips are intentionally NOT passed here —
 * leaving them unstamped keeps them queued for a later retry, matching the
 * Wikidata worker's handling.
 *
 * @param {string[]} spotifyIds
 * @param {object} [client]
 */
export async function markTracksMusicbrainzChecked(spotifyIds, client = createAdminClient()) {
    if (spotifyIds.length === 0) return;
    ...
    for (const batch of chunk(spotifyIds, QUERY_CHUNK)) {
        const { error } = await client
            .from("tracks")
            .update({ musicbrainz_checked_at: checkedAt })
            .in("spotify_id", batch);
        if (error) throw new Error(`markTracksMusicbrainzChecked failed: ${error.message}`);
    }
}
...
export async function updateTrackIsrcAndMbid(
    ...
    const patch = {
        // Mark MusicBrainz as consulted so this row leaves the backfill queue
        // (getTracksMissingIsrc) even when no ISRC came back — otherwise an
        // mbid-only row stays isrc-NULL and gets re-fetched forever.
        musicbrainz_checked_at: new Date().toISOString(),
...
export async function getTracksMissingIsrc(
    ...
    // Queue = tracks MusicBrainz hasn't been consulted about yet. The
    // musicbrainz_checked_at marker (migration 0013) is what lets MBID-only and
    // not-found rows leave the queue — filtering on isrc alone re-fetched them
    // forever (they stay isrc-NULL). Backed by tracks_mb_unchecked_idx.
