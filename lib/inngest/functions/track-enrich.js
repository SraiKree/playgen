...
export const trackEnrich = inngest.createFunction(
  {
    ...
    // Shared Last.fm budget: track-enrich + artist-enrich draw from ONE virtual
    // "lastfm" queue (scope: env) so their COMBINED in-flight Last.fm calls stay
    // <= 5 (CLAUDE.md §5). A plain { scope: "fn", limit: 5 } would let each
    // function have its own 5, allowing 10 concurrent calls once the floor lands.
  ...
  async ({ event, step, logger }) => {
    ...
    // Fan out to the artist genre floor (Layer 1.5). `artists` is the deduped
    // set from this batch; the artist-enrich worker re-filters to those still
    // needing a Last.fm pass (tags_enriched_at IS NULL), so re-emitting an
    // already-floored artist is a harmless no-op. This runs in parallel with the
    // Wikidata fan-out at the end — both are independent Layer-1 sources. No
    // jobId: the floor is a background side-effect, not part of the playlist
    // job's track-enrichment progress accounting.
    if (artists.length > 0) {
