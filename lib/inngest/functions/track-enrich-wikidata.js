...
import { fetchTrackTagsByIsrc } from "@/lib/wikidata";
import {
...
export const trackEnrichWikidata = inngest.createFunction(
  ...
  async ({ event, step, logger }) => {
    ...
    for (const { spotifyId, isrc } of todo) {
      // ISRC-less tracks: skip the fetch entirely. We still mark these
      // enriched below so the worker queue keeps draining (see top-of-file
      // comment for the reasoning) — that's why they go into
      // idsToMarkEnriched instead of being treated as bad data.
      if (!isrc) {
        isrcMissing += 1;
      ...
      // step.run memoizes the result — on retry the SPARQL call is skipped
      // and the upsert step picks up from the cached payload. The helper
      // returns a discriminated result on bad data (instead of throwing) so
      // one poisoned ISRC can't crash the whole batch.
      const result = await step.run(`wikidata-${spotifyId}`, () =>
        fetchTrackTagsByIsrc(isrc),
      );
