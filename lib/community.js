...
/**
 * Suggested (below-threshold) tags for one track.
 *
 * Mirror of getCommunityTagsForTrack but inverts the score filter — returns
 * every visible tag whose score has NOT yet reached the graduation threshold.
 * Surfacing these to every viewer (not just the submitter) is what lets new
 * tags accumulate the upvotes they need to graduate; otherwise nobody outside
 * the original submitter could vote a fresh tag past the threshold.
 *
 * No "rejection" — negative-score tags stay visible. Soft-hidden tags
 * (`hidden = true`) are still excluded, matching community-tag behavior.
 *
 * Returns array sorted by score desc, then tag name asc (sortTagsByScore).
 *
 * @param {object} client
 * @param {{trackId: number, currentUserId?: string|null, threshold?: number}} params
 */
export async function getSuggestedTagsForTrack(
    client,
    { trackId, currentUserId = null, threshold = 2 },
) {
    toPositiveInt(trackId, "trackId");
    ...
    const { data: scoreRows, error } = await client
        ...
        .eq("hidden", false)
        .lt("score", threshold);
    if (error) throw new Error(`getSuggestedTagsForTrack failed: ${error.message}`);
    if (scoreRows.length === 0) return [];
...
/**
 * Batched version of getSuggestedTagsForTrack. Returns Map<trackId, entries[]>.
 * Tracks with no below-threshold tags are absent from the map.
 *
 * @param {object} client
 * @param {{trackIds: number[], currentUserId?: string|null, threshold?: number}} params
 * @returns {Promise<Map<number, Array<object>>>}
 */
export async function getSuggestedTagsForTracks(
    client,
    { trackIds, currentUserId = null, threshold = 2 },
) {
    if (!Array.isArray(trackIds) || trackIds.length === 0) return new Map();
    ...
    for (const batch of chunk(trackIds, QUERY_CHUNK)) {
        const { data, error } = await client
            ...
            .eq("hidden", false)
            .lt("score", threshold);
        if (error) throw new Error(`getSuggestedTagsForTracks failed: ${error.message}`);
        for (const row of data) allRows.push(row);
    }
    if (allRows.length === 0) return new Map();
