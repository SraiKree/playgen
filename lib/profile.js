...
/**
 * Map average Spotify popularity (0–100) to a human descriptor on the
 * mainstream↔underground axis.
 */
export function popularityDescriptor(avg) {
    const p = Number(avg) || 0;
    if (p < 25) return "deep underground";
    if (p < 45) return "off the beaten path";
    if (p < 65) return "a balanced palate";
    if (p < 80) return "crowd-tested";
    return "firmly mainstream";
}
...
export async function getProfilePageData(client, userId) {
    const [
        ...
        callRpc(client, "taste_top_genres", { p_limit: 8 }),
    ...
    return {
        ...
        overview: {
            ...
            avgPopularity: Number(overview.avg_popularity),
